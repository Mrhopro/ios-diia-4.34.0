    ...
    private func fetchDocs(in group: DispatchGroup) {
        ...
        apiClient.getDocuments(irrelevantDocs).observe { [weak self] (event) in
            ...
            case .completed:
                break
            case .failed:
                if self.storeHelper.getValue(forKey: .driverLicense) as DSFullDocumentModel? == nil,
                   let mockDocs = MockDocumentsData.mockFullDocumentModel {
                    self.saveDoc(mockDocs, type: DSFullDocumentModel.self, forKey: .driverLicense, orderType: .driverLicense)
