...
final class PublicServiceOpener: PublicServiceOpenerProtocol {
    ...
    */
    ...
    func openPublicService(_ publicService: PublicServiceShortViewModel, in view: any BaseView) {
        if let serviceRoute = serviceRouteManager.routeFor(publicService) {
            serviceRoute.route(in: view)
            return
        }

        let detailVC = FeatureDetailViewController(
            title: publicService.title,
            subtitle: "Послуга доступна для отримання в електронній формі через застосунок та портал Дія.",
            badgeText: "ДЕРЖАВНА ПОСЛУГА",
            sections: [
                (
                    title: "Як отримати послугу",
                    items: [
                        ("Крок 1", "Перевірте коректність персональних даних"),
                        ("Крок 2", "Заповніть електронну форму та додайте необхідні відомості"),
                        ("Крок 3", "Підпишіть заяву за допомогою Дія.Підпису")
                    ]
                ),
                (
                    title: "Строк та вартість",
                    items: [
                        ("Строк надання", "Від кількох хвилин до 1 робочого дня"),
        ...
        struct GenericDetailModule: BaseModule {
            let vc: UIViewController
            func viewController() -> UIViewController { vc }
        }
        view.open(module: GenericDetailModule(vc: detailVC))
    }
